...
System.out.println(dateFormat1.format(date1));
		...
		pst.setString(7,dateFormat1.format(date1));
		pst.execute();
		updatecount(orderName);
	}
	catch(Exception e)
	{
...
public static void updatecount(String orderName)
{
	try
	{
		getConnection();
		String insertIntoCustomerOrderQuery = "update productDetails set AvailableCount= AvailableCount-1 where productName=?";
		PreparedStatement pst=conn.prepareStatement(insertIntoCustomerOrderQuery);
